...
When I say "send to Claude Code", leave a comment addressed to Claude (field to: ["claude"]) in the document voice-mode-for-claude-code (link: https://claude.ai/code/artifact/${config.docId}; document id: ${config.docId}; tab body id: ${tab.content.id}), containing the request I say next. When I say "read Claude Code's answer", read aloud Claude's latest reply in that document.
`);
